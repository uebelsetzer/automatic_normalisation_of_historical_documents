...
A simple way to do create a new environment would be to start within the folder with this repository and type:

```
python3 -m venv nmt4hd # this can be any name
source nmt4hd/bin/activate # this will activate the newly created evironment, so whatever Python package installed after the activation only works within the environment
```

This should activate the new environment, and make sure it will not mess with anything else in the root Python environment.
...
```
python3 finetuningNMT-opus.py -m <path_to_model> -d <path_to_dataset_folder> -dn <name_of_the_dev_file> -tn <name_of_the_train_file>
```

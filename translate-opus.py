...
###########################################################
def arguments():
    parser = argparse.ArgumentParser(description="Translate using OPUS model")
    parser.add_argument("-m", "--model", default="models/opus-mt-tc-big-itc-itc-finetuned-por-to-pob/checkpoint-297", help="Folder containing an OPUS MT model. Or path to an existing model online (for instance, from Huggingface).")
    parser.add_argument("-s", "--source", default="test_sets/medical_test.txt", help="File containing one sentence per line.")
    parser.add_argument("-o", "--output", default="results/medical_opus_ft.txt", help="Output file.")
    ...
    parser.add_argument("-l", "--language", default="", help="Language tag for the translation task. Should be provided in the format >>id<< (for instance, >>pob<<). It does not work well for fine-tuned models.")
    ...
    for src_text in src_list:
        ...
        if is_too_long:
            ...
            for st in src_text:
                ...
                translated_text = model.generate(**s)
            ...
            src_text = lang + src_text # This line actually makes it worse for the fine-tuned models
